...
$home = "/home/sites/police/bot_blacklist";
$logs = "$home/logs";
...
unless ($home eq ".") {
	...
	if (open my $file, "$home/feebot.pid") {
		my $pid = <$file>;
		chomp $pid;
		close $file;
		exit if -d "/proc/$pid";
...
#				$sock = IO::Socket::INET->new(PeerAddr => "city1.timezero.ru", PeerPort => 5191);
...
#					$sock = IO::Socket::INET->new(PeerAddr => "chat.timezero.ru", PeerPort => 5191);
#					$sock = IO::Socket::INET->new(PeerAddr => "213.79.68.180", PeerPort => 5190);
	...
	foreach my $sock ($select->can_read(0.2)) {
		...
		unless(defined $flag && length $data) {
			...
			} elsif ($sock == $bot->{chat}{sock} && length($data) == 0){
				#в чате пустые данные могут быть
