...
require("_misc_funcs.php");
	...
	print "<center><h1>Статистика следователя ".AuthUserName.".</h1></center>\n";
	$template_type=array();
	...
	$result=mysql_query("SELECT UNIX_TIMESTAMP(date_case_begin) as date_case_begin,UNIX_TIMESTAMP(date_case_taken) as date_case_taken,UNIX_TIMESTAMP(date_case_closed) as date_case_closed FROM case_main WHERE investigator=\"".AuthUserName."\" and date_case_closed between $dates_between;");
	...
	{	
		...
		$cases_solved=0;
        $result2=mysql_query("SELECT count(case_id) as cases_current FROM case_main WHERE investigator=\"".AuthUserName."\" and date_case_closed = \"0000-00-00 00:00:00\" and case_type".(isset($_REQUEST["case_type"])?($_REQUEST["case_type"]>2?">2":"=".$_REQUEST["case_type"]):"=1").";");
        $row2=mysql_fetch_assoc($result2);
		$cases_current=$row2["cases_current"];
		$result2=mysql_query("SELECT count(case_id) as cases_taken FROM case_main WHERE investigator=\"".AuthUserName."\" and date_case_taken between ".$dates_between." and case_type".(isset($_REQUEST["case_type"])?($_REQUEST["case_type"]>2?">2":"=".$_REQUEST["case_type"]):"=1").";");
		$row2=mysql_fetch_assoc($result2);
	    ...
	    $result2=mysql_query("(SELECT 'untemplated' as template,count(case_id) as resultset from case_main where date_case_closed between ".$dates_between." and investigator=\"".AuthUserName."\" and closure_type=0) union (SELECT '0',count(case_id) as resultset from case_main where date_case_closed between ".$dates_between." and investigator=\"".AuthUserName."\" and closure_type=1) union (SELECT '1',count(case_id) as resultset from case_main where date_case_closed between ".$dates_between." and investigator=\"".AuthUserName."\" and closure_type=2)");
