...
$res = mysql_query("SELECT * FROM `online_smart` WHERE `logout` < 1");
$curdate = date("Y-m-d");
$curtime = time();
$count = 0;
	...
	foreach ($curcops as $key => $tmp)
		{
			...
			$query = "UPDATE `online_smart` SET `logout`='".$tmptimestamp."' WHERE `id` = '".$tmp['id']."' LIMIT 1;";
			mysql_query($query);
			$tmptimestamp = $tmptimestamp+60;
			$query = "INSERT INTO `online_smart` SET `nick` = '".$key."', `login` = '".$tmptimestamp."', `date` = '".$curdate."', `status` = '".$tmp['status']."'";
			mysql_query($query);
		}
	}
foreach (glob("/home/sites/police/bots/smarts_monitor/logs/smart-*.txt") as $filename)
	{
		$curptime = strptime($filename, "/home/sites/police/bots/smarts_monitor/logs/smart-%Y%m%d%H%M%S.txt");
echo ("<pre>");
print_r ($curptime);
echo ("</pre>");
...
//				if ($nick !== "")
		...
		//		if (strpos($arr[1],"+"))
			...
			//////////////////////////////////
				if ($tempvar > 0 && $tempvar !== $curdate)
					{
						$query = "INSERT INTO `online_smart` SET `nick` = '".$nick."', `login` = '".time()."', `logout`='".(time()+1)."', `date` = '".$curdate."', `status` = '0'";
						mysql_query($query);
					}
			//////////////////////////////////
			}
		...
		if ($curcops[$key]['login'] > 0 && $value == 1)
			{
				if ($curcops[$key]['status'] == 1 && $curcops[$key]['done'] == 0)
					{
						$query = "UPDATE `online_smart` SET `logout`='".$curtime."' WHERE `id` = '".$curcops[$key]['id']."' LIMIT 1;";
						mysql_query($query);
						$query = "INSERT INTO `online_smart` SET `nick` = '".$key."', `login` = '".$curtime."', `date` = '".$curdate."', `status` = '0'";
						mysql_query($query);
						$curcops[$key]['done'] = 1;			
					}
				elseif ($value == 1)
				...
				if (($curcops[$key]['status'] == 0) && $curcops[$key]['done'] == 0)
					{
						$query = "UPDATE `online_smart` SET `logout`='".$curtime."' WHERE `id` = '".$curcops[$key]['id']."' LIMIT 1;";
						mysql_query($query);
						$query = "INSERT INTO `online_smart` SET `nick` = '".$key."', `login` = '".$curtime."', `date` = '".$curdate."', `status` = '1'";
						mysql_query($query);
			 			$curcops[$key]['done'] = 1;
...
foreach($curcops as $tmp)
	{
		if($tmp['done'] == 0)
			{
				$query = "UPDATE `online_smart` SET `logout`='".$curtime."' WHERE `id` = '".$tmp['id']."' LIMIT 1;";
				mysql_query($query);
			}
	}
?>
