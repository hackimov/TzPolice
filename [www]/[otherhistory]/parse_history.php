...
$logparams = explode('|',$_REQUEST['logno']);
	...
	if ($listhistories)	{
		...
		if ($userfiles) {
			...
			if (array_key_exists(AuthUserName,$userfiles)) {
				print "Ваши запросы<br>";
				foreach($userfiles[AuthUserName] as $logno=>$logname) {
					preg_match("/\[(.+)\] \[(.+)\] (\d+\.\d+\.\d+) (\d+\.\d+\.\d+)\s?(\d?)\.txt/",$logname,$matches);
					print "[<a href='/otherhistory/download.php?logno=".AuthUserName."|".$logno."'>ZIP</a>] <a href=\"".(strpos($_SERVER['QUERY_STRING'],"longlogs")!==false?"?act=longlogs&":"?")."logno=".AuthUserName."|$logno|".$matches[5]."\">История ".$matches[2]." за период с ".$matches[3]." по ".$matches[4]."</a>";
                    //print "[".($matches[5]<1?'история жизни':'логи боёв')."]";
					print "<br />";
				}
				print "<hr>";
			}
        	foreach ($userfiles as $username=>$logfiles)
			{
				...
				{
					preg_match("/\[(.+)\] \[(.+)\] (\d+\.\d+\.\d+) (\d+\.\d+\.\d+)\s?(\d?)\.txt/",$logname,$matches);
					print "[<a href='/otherhistory/download.php?logno=".$username."|".$logno."'>ZIP</a>] <a href=\"".(strpos($_SERVER['QUERY_STRING'],"longlogs")!==false?"?act=longlogs&":"?")."logno=$username|$logno|".$matches[5]."\">Просмотр заказан ".$matches[1].", история ".$matches[2]." за период с ".$matches[3]." по ".$matches[4]."</a>";
					//print "[".($matches[5]<1?'история жизни':'логи боёв')."]";
					print "<br />";
				}
				if ($admin_access)
...
function ParseLogFile($admin_access,$dept_list,$logcount,$logfilter) {
	...
	arsort($userfiles,SORT_STRING);
	list($username,$logno,$ltype)=explode('|',$logcount);
	$logfile=@fopen($userfiles[$username][$logno],"r");
	preg_match("/\[(.+)\] \[(.+)\] (\d+\.\d+\.\d+) (\d+\.\d+\.\d+)\s?(\d?)\.txt/",$userfiles[$username][$logno],$matches);
	echo "История персонажа <b>$matches[2]</b> c $matches[3] по $matches[4] | <b>$matches[1]</b>
	<hr />
	<script>
	document.getElementById('lognick').value='".$matches[2]."';
	document.getElementById('lognick2').value='".$matches[2]."';
	document.getElementById('logtext2').value='[".$matches[1]."] [".$matches[2]."] ".$matches[3]." ".$matches[4]."';
	</script>";
	if ($logfile)
	{
		echo ('<div id="logf">');
...
if(AuthUserClan=='Tribunal' || AuthUserClan=='police' || AuthUserClan=='Police Academy' || AuthUserGroup==100) {
	$sSQL = "SELECT `dept`, `chief` FROM `sd_cops` WHERE `name` = '".AuthUserName."' ORDER BY `chief` DESC LIMIT 1;";
...
function processlog(){
	...
	if(!logType) {
		...
		//alert('Только для логов боёв');
	}
	document.getElementById('sendlog').submit();
}
function processfil(type){
	if(type < 1) {
		document.getElementById('logfilter').value='';
	} else {
		var ftext = document.getElementById('logfilter').value;
		if(ftext.length < 1) {
			alert('Введите фильтруемый текст');
			return;
		}
	}
	document.getElementById('filterform').submit();
}
function processlog2(){
	alert('Тестовый режим. Все пожелания системкой на Deorg');
	if(!logType) {
		//alert('Только для истории жизни');
	} else {
		//document.getElementById('sendlog2').submit();
	}
	document.getElementById('sendlog2').submit();
}
</script>
...
<?
	...
	if(isset($_REQUEST['logno'])) {
		...
		if(abs(AccessLevel) & AccessOP)	{
			echo "
			<table style='width: 100%; margin-bottom: 15px; margin-top: 15px; font-size: 12px;' cellpadding=3 cellspacing=1 border=0>
			<tr>
				<th style='border: 2px solid #deceb4;'>Скачать</th>
				<th style='border: 2px solid #deceb4;'>Фильтр (слова через запятую и пробел)</th>
				<th style='border: 2px solid #deceb4;'>Анализатор боёв персонажа</th>
				<th style='border: 2px solid #deceb4;'>Анализатор ресурсов каторжника</th>
			</tr>
			<tr>
				<th style='border: 2px solid #deceb4;'><a href='/otherhistory/download.php?logno=".$_REQUEST['logno']."'>Скачать лог в архиве</a></th>
				<th style='border: 2px solid #deceb4;'>
					<form method=POST action=\"".(strpos($_SERVER['QUERY_STRING'],"longlogs")!==false?"?act=longlogs&":"?")."logno=$logno\" id=\"filterform\" name=\"filterform\" onSubmit=\"setCookie('tzpd_lfc',document.getElementById('logfilter').value,'Fri, 01-Jan-2055 00:00:00 GMT','/')\">
						<input type=text name=\"logfilter\" value=\"$logfilter\" id=\"logfilter\">
						<input type=button value=\"фильтр\" onclick='processfil(1);'>
						<input type=button value=\"сброс\" onclick='processfil(0);'>
					</form>
				</th>
				<th style='border: 2px solid #deceb4;'>
					<form method=POST action='?act=prokachki&d=edit' name='F1' id='sendlog' target='_blank'>
            			...
            			<input type=submit value='Отправить' onClick='processlog(); return false;'>
					</form>
				</th>
				<th style='border: 2px solid #deceb4;'>
					<form method=POST action='?act=aprisone' name='F2' id='sendlog2' target='_blank'>
						<input type='hidden' name='history' id='logtext2'>
            			<input type='hidden' name='go' value='1'>
						<input type='text' name='nickname' id='lognick2' VALUE='Подозреваемый'>
            			<input type=submit value='Отправить' onClick='processlog2(); return false;'>
					</form>
				</th>
			</tr>
			</table>
