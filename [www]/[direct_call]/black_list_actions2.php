...
//                echo ($u_term);
	...
	$add_ts = time();
		...
		$u_updated = time();
        $query = 'UPDATE `police_b_list2` SET `added` = \''.$add_ts.'\', `reason` = \''.$u_reason.'\', `term` = \''.$u_term.'\', `status` = \'0\', `payment` = \'\', `no_payment_till` = \'0\', `payment_till` = \'0\' WHERE `nick` = \''.$u_nick.'\' LIMIT 1;';
//        echo ($query);
...
if ($section == "edit" && @$_REQUEST['nick'])
	{
		...
		$query = 'SELECT `level`, `clan`, `pro`, `reason`, `term` FROM `police_b_list2` WHERE `nick` = \''.$u_nick.'\' LIMIT 1;';
...
if ($section == "do_edit")// && @$_REQUEST['nick'] && @$_REQUEST['term'] && @$_REQUEST['old_term'] && @$_REQUEST['reason'])
	{
        ...
        $u_reason = str_replace("\r\n", '<br>', $u_reason);
        $query = "UPDATE `police_b_list2` SET `reason` = '".$u_reason."', `term` = '".$u_term."' WHERE `nick` = '".$u_nick."' LIMIT 1;";
        mysql_query($query);
        ?>
