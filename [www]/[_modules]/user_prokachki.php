...
<h1>Пользователи форума отдела контроля прокачек</h1> <?
		...
		if(@$_REQUEST["del"])

        	{
                ...
                $cur_restr = str_replace("-prokachki-", "", $cur_restr);
		...
		$query = "SELECT `user_name`, `restricted_area` FROM `site_users` WHERE `restricted_area` LIKE '%prokachki%' ORDER BY `user_name`";
...
<form name="add_form" method="post" action="?act=user_prokachki">
...
//                    ."<td $bg[$y]>$area[$u_restr]</td>"
                    ...
                    ."<td $bg[$y]><a href='?act=user_prokachki&del=$u_name' onClick=\"if(!confirm('Вы уверены?')) return false;\">Убрать доступ</a></td></tr>";
