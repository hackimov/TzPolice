<h1>Интернет-магазин "военторгЪ Traders edition"</h1>
...
if (AuthStatus==1 && (substr_count(AuthUserRestrAccess, "traders_shop") > 0 || substr_count(AuthUserRestrAccess, "traders_shop") > 0))
	{
	    ...
	    if(strlen($_REQUEST['order'])>3)
        	{
	            ...
	            if($error)
                	{
	                	echo "<script>top.location='?act=shop_traders&error=".$error."';</script>";
		            }
                else
                	{
	                    $SQL="INSERT INTO traders_orders (customer, content, resources, factory, t1, res_loc)
	                    values('".AuthUserId."','".$items."','".$resources."','".$_REQUEST['f']."','".time()."','".addslashes(strip_tags($_REQUEST['where_res']))."')";
                        ...
                        echo "<script>top.location='?act=shop_traders';</script>";
		            }
	    	}
	    ...
	    function MakeOrder(a) {
	        ...
	        for(var i=1;i<items[a].length;i++) {
           	...
           	if (pl = window.prompt("Укажите, где находятся ресурсы", "")) {
				if (!pl) alert("Необходимо указать место, где находятся ресурсы!")
				else {
	        if(req.length>1) {
	            if(confirm('Вы уверены в своем заказе?')) top.location='?act=shop_traders&where_res='+pl+'&order='+req+'&f='+a+'&t='+document.all('Rtime'+a).value;
	        } else {
	    ...
	    <? if($permission) { ?>
	            ...
	            <form>
	            <input name="act" type="hidden" value="shop_traders">
	    ...
	    2. Обязательно указывайте местоположение ресурсов, предназначенных для производства заказа<br>
	    3. После получения товара необходимо ставить пометку об этом
	    </td></tr>
	    </table>
	    </center>
	    ...
	    ?>
	            <p><img src='i/bullet-red-01a.gif' width='18' height='11' hspace='5'><strong>For Co6aKa_2 only:</strong>
	            <br><br>
	            <a href="direct_call/shop_history.php?item=all_traders" target="logs">Просмотреть сводные логи</a>
	            <center>
	    ...
	    foreach (array_keys($factories) as $fid)
        	{
	            ...
	            <?
	            $SQL="SELECT * FROM shop_items WHERE is_visible='1' AND `available`>'0' AND factory='".$fid."' ORDER BY `group`, `id` DESC";
	            $r=mysql_query($SQL);
	            $np=0;
	            $iclass='';
	            ...
	            while($d=mysql_fetch_array($r))
                	{
	                    ...
	                    <td <?=$bg?>><?=$d['item_name']?><?if($permission) echo " (<a href='#; return false;' onclick=\"if(confirm('Скрыть предмет?')) top.location='?act=shop_traders&hide={$d[id]}';\">X</a>)"?>&nbsp;<?if(AuthUserGroup == 100) echo " (<a href='direct_call/shop_history.php?item={$d[id]}&k=t' target='logs'>Логи</a>)"?></td>
	    ...
	    <tr><form name="prodid_<?=$d['id']?>" action="?act=shop_traders" method="POST">
	    <input name="editID" type="hidden" value="<?=$d['id']?>">
	    <td <?=$bg?>><input name="ProductName" type="text" value="<?=$d['item_name']?>" size=40>
	    <?echo " (<a href='#; return false;' style='color: green' onclick=\"if(confirm('Вернуть предмет?')) top.location='?act=shop_traders&show={$d[id]}';\">X</a>)"?>
	    <?echo " (<a href='#; return false;' onclick=\"if(confirm('Удалить предмет?')) top.location='?act=shop_traders&del={$d[id]}';\">X</a>)"?>&nbsp;<?if(AuthUserGroup==100) echo " (<a href='direct_call/shop_history.php?item={$d[id]}&k=t' target='logs'>Логи</a>)"?></td>
