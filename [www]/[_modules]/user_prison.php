...
if(AuthUserClan=='police' || AuthUserGroup == 100) {
?>
<h1>Пользователи учета ресов с каторги</h1> <?
		if(@$_REQUEST["del"])
        	{
            	$query = "SELECT `restricted_area` FROM `site_users` WHERE `user_name` = '".$_REQUEST["del"]."' LIMIT 1;";
                $rs = mysql_query($query);
                list($cur_restr) = mysql_fetch_row($rs);
                $cur_restr = str_replace("-prison-", "", $cur_restr);
		...
		$query = "SELECT `user_name`, `restricted_area` FROM `site_users` WHERE `restricted_area` LIKE '%-prison-%' ORDER BY `user_name`";
...
<form name="add_form" method="post" action="?act=user_prison">
...
//                    ."<td $bg[$y]>$area[$u_restr]</td>"
                    ."<td $bg[$y]><a href='?act=user_prison&del=$u_name' onClick=\"if(!confirm('Вы уверены?')) return false;\">Убрать доступ</a></td></tr>";
                $y++;
                if ($y>1) {$y = 0;}
            }
        ?>
