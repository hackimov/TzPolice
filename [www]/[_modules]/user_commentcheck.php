		...
		if(@$_REQUEST["nick"])
        	{
                ...
                $rs = mysql_query($query);
                list($cur_restr) = mysql_fetch_row($rs);
                $cur_restr .= "-commentcheck-";
            	$query = "UPDATE `site_users` SET `restricted_area` = '".$cur_restr."' WHERE `user_name` = '".$_REQUEST["nick"]."' LIMIT 1";
                mysql_query($query);
            }
		$query = "SELECT `user_name`, `restricted_area` FROM `site_users` WHERE `restricted_area` LIKE '%commentcheck%' ORDER BY `user_name`";
...
<form name="add_form" method="post" action="?act=user_commentcheck">
...
//                    ."<td $bg[$y]>$area[$u_restr]</td>"
                    ."<td $bg[$y]><a href='?act=user_commentcheck&del=$u_name' onClick=\"if(!confirm('Вы уверены?')) return false;\">Убрать доступ</a></td></tr>";
                $y++;
                if ($y>1) {$y = 0;}
            }
        ?>
