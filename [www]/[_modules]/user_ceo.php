...
?>
<h1>Пользователи форума для нач. отделов</h1>
<?
		if(@$_REQUEST['del']) {
			...
			$cur_restr = str_replace('-ceo-', '', $cur_restr);
		...
		$query = "SELECT `user_name`, `restricted_area` FROM `site_users` WHERE `restricted_area` LIKE '%ceo%' ORDER BY `user_name`";
...
<form name="add_form" method="post" action="?act=user_ceo">
...
<?
		...
		while (list($u_name, $u_restr) = mysql_fetch_row($rez)) {
			echo '<tr><td '.$bg[$y].'>'.$u_name.'</td>';
//			."<td $bg[$y]>$area[$u_restr]</td>"
			echo '<td '.$bg[$y].'><a href="?act=user_ceo&del='.$u_name.'" onClick="if(!confirm(\'Вы уверены?\')) return false;">Убрать доступ</a></td></tr>';
			$y++;
			if ($y>1) {$y = 0;}
		}
?>
