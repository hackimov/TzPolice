		...
		if (pg > 1)
        	{
	            ...
	            if(pg > 6){
	                for(z = 1; z<=3; z++){txt += '<a href="'+link+'&p='+z+'">'+z+'</a> ';}
	                txt += '... ';
	                for(z = pg -2; z<=pg; z++){txt += '<a href="'+link+'&p='+z+'">'+z+'</a> ';}
	            }else{
	                for(z = 1; z<=pg; z++){txt += '<a href="'+link+'&p='+z+'">'+z+'</a> ';}
	            }
	            txt += '</div>';
            }
        else
...
<?
    ...
    if ($_REQUEST['search'])
    {      $word = urldecode($_REQUEST['search']);
      ...
      $SQL="SELECT th.*, u.id AS user_id, u.user_name AS user_name, u.clan
         	FROM freelance_threads th
         	LEFT JOIN site_users u ON (th.poster_id=u.id)
         	LEFT JOIN freelance_replies re ON (th.id=re.id_thread)
    ...
    if(mysql_num_rows($r)>0) {
        ...
        ShowPages($p,$pages,5,"act=freelance_threads&f={$_REQUEST['f']}");
        echo "</b></div>";
?>
...
//Тута мы посылаем "неэлитную" часть ментов =)
        ...
        $ThId=$d['id'];
        $repl=mysql_fetch_array(mysql_query("SELECT count(id) FROM freelance_replies WHERE id_thread='$ThId'"));
        if ($d['allow_comments']==0) echo "[closed] ";
        echo "
        	<a href='?act=freelance_replies&f={$f}&th={$d['id']}'>".stripslashes($d['title'])."</a></h4>
			<script language='javascript'>document.write(pages('".ceil($repl[0]/$RepliesPP)."', '?act=freelance_replies&f={$f}&th={$d['id']}'));</script>
			</td><td background='i/bgr-grid-sand1.gif' nowrap class='v9' align=center>
...
<?		}}?>
        ...
        <input type="hidden" name="act" value="freelance_threads">
...
?>
	...
	<?
    foreach($freelance as $ForumId => $f) {
...
if ($f['restr'] == 0 || substr_count(AuthUserRestrAccess, $f['nm']) > 0 || AuthUserGroup==100)
        	{
		    	$last_th=mysql_fetch_array(mysql_query("SELECT MAX(last_date) FROM freelance_threads WHERE id_sec='$ForumId'"));
				$cnt_th=mysql_fetch_array(mysql_query("SELECT count(id) FROM freelance_threads WHERE id_sec='$ForumId'"));
		    ?>
				<tr>
		        <td width=100% background='i/bgr-grid-sand.gif'>
	    		    <h3><a href='?act=freelance_threads&f=<?=$ForumId?>'><?=$f['name']?></a></h3>
