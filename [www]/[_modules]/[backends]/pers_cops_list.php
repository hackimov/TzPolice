...
<?
		$query = "SELECT `id`, `pers` FROM `or_data` WHERE `status` = '1' ORDER BY `id`";
		$res = mysql_query($query);
		while ($ttmp = mysql_fetch_array($res)) {
			echo '<option value="'.$ttmp['id'].'">'.$ttmp['pers'].'</option>';
		}
?>
  </select>
...
<?
		...
		$_RESULT = array("res" => "ok");
		echo 'user validation failed. terminating.';
	}
} else {
	$_RESULT = array("res" => "ok");
	echo 'cannot get session id. terminating.';
}
