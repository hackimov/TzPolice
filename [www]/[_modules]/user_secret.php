...
<h1>Пользователи Top Secret</h1> <?
		...
		if(@$_REQUEST["del"])

        	{
                ...
                $cur_restr = str_replace("-secret-", "", $cur_restr);
		...
		$query = "SELECT `user_name`, `restricted_area` FROM `site_users` WHERE `restricted_area` LIKE '%secret%' ORDER BY `user_name`";
...
<form name="add_form" method="post" action="?act=user_secret">
...
//                    ."<td $bg[$y]>$area[$u_restr]</td>"
                    ...
                    ."<td $bg[$y]><a href='?act=user_secret&del=$u_name' onClick=\"if(!confirm('Вы уверены?')) return false;\">Убрать доступ</a></td></tr>";
