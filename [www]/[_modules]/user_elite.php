...
if((abs(AccessLevel) & AccessForumUMO) || AuthUserGroup == 100) {
?>
<h1>Пользователи Top Secret - Экономический отдел</h1> <?
		if(@$_REQUEST["del"])
        	{
            	$query = "SELECT `restricted_area` FROM `site_users` WHERE `user_name` = '".$_REQUEST["del"]."' LIMIT 1;";
                $rs = mysql_query($query);
                list($cur_restr) = mysql_fetch_row($rs);
                $cur_restr = str_replace("-elite-", "", $cur_restr);
		...
		$query = "SELECT `user_name`, `restricted_area` FROM `site_users` WHERE `restricted_area` LIKE '%elite%' ORDER BY `user_name`";
		$rez = mysql_query($query);
        $area[1] = "Доступ к закрытому форуму";
...
<form name="add_form" method="post" action="?act=user_elite">
...
//                    ."<td $bg[$y]>$area[$u_restr]</td>"
                    ."<td $bg[$y]><a href='?act=user_elite&del=$u_name' onClick=\"if(!confirm('Вы уверены?')) return false;\">Убрать доступ</a></td></tr>";
                $y++;
                if ($y>1) {$y = 0;}
            }
        ?>
