...
<h1>Пользователи форума ОБЭП</h1> <?
		...
		if(@$_REQUEST["del"])

        	{
                ...
                $cur_restr = str_replace("-obep-", "", $cur_restr);
		...
		$query = "SELECT `user_name`, `restricted_area` FROM `site_users` WHERE `restricted_area` LIKE '%obep%' ORDER BY `user_name`";
...
<form name="add_form" method="post" action="?act=user_obep">
...
//                    ."<td $bg[$y]>$area[$u_restr]</td>"
                    ...
                    ."<td $bg[$y]><a href='?act=user_obep&del=$u_name' onClick=\"if(!confirm('Вы уверены?')) return false;\">Убрать доступ</a></td></tr>";
