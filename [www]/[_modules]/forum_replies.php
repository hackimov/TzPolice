...
if (abs(AccessLevel) & AccessEditForum)
	{
		if ($_REQUEST['step'] == "save_edit")
			{
				$savetext = addslashes($_REQUEST['NewsText']);
				if ($_REQUEST['do'] == 'edit')
					{
						$qe = "UPDATE `forum_replies` SET `text` = '".$savetext."' WHERE `id` = '".$_REQUEST['postid']."' LIMIT 1;";
						$rse = mysql_query($qe);
						echo "<script>top.location.href='?act=forum_replies&f=".$_REQUEST['f']."&th=".$_REQUEST['th']."&p=".$_REQUEST['p']."'</script>";
					}

				else if ($_REQUEST['do'] == 'editthread')
					{
						$qe = "UPDATE `forum_threads` SET `text` = '".$savetext."' WHERE `id` = '".$_REQUEST['th']."' LIMIT 1;";
						$rse = mysql_query($qe);
						echo "<script>top.location.href='?act=forum_replies&f=".$_REQUEST['f']."&th=".$_REQUEST['th']."&p=".$_REQUEST['p']."'</script>";
					}
...
if (abs(AccessLevel) & AccessForum) {
	if($_REQUEST['f'] && $_REQUEST['th']) {
		...
		if ($forum[$f]['restr'] < 1 || substr_count(AuthUserRestrAccess, $tmp) > 0 || AuthUserGroup==100) {
			...
			$SQL = 'SELECT th.*, u.id AS user_id, u.user_name AS user_name, u.clan FROM `forum_threads` th LEFT JOIN site_users u ON th.poster_id=u.id WHERE th.id_sec="'.$f.'" AND th.id="'.$th.'";';
			$r=mysql_query($SQL);
			if(mysql_num_rows($r)>0) {
				$thread = mysql_fetch_array($r);
				mysql_query('UPDATE `forum_threads` SET `cnt_views`=`cnt_views`+1 WHERE `id`="'.$th.'"');
				echo '<h1>'.stripslashes($thread['title']).'</h1>';
			...
			#
				...
				//определили
					...
					$SQL = 'INSERT INTO `forum_replies` (`id_thread`, `poster_id`, `post_date`, `text`) VALUES ("'.$th.'", "'.AuthUserId.'", "'.time().'", "'.addslashes($DelText).'")';
					mysql_query($SQL);
					
					$SQL = 'UPDATE `forum_threads` SET `allow_comments`="0" WHERE `id`="'.$th.'"';
					mysql_query($SQL);
					
					echo '<script>top.location.href=\'?act=forum_threads&f='.$f.'\'</script>';
				...
				//определили
					...
					$SQL = 'UPDATE `forum_threads` SET `allow_comments`="1" WHERE id="'.$th.'"';
					mysql_query($SQL);
					echo '<script>top.location.href=\'?act=forum_threads&f='.$f.'\'</script>';
...
//определили
	        ...
	        $SQL="UPDATE forum_replies SET text='".addslashes($DelText)."' WHERE id='".$_REQUEST['postid']."'";
			mysql_query($SQL);
			echo "<script>top.location.href='?act=forum_replies&f={$f}&th={$th}&p={$p}'</script>";
        } elseif(@$_REQUEST['do']=="attach" && $megaboss>0) {
			$SQL="UPDATE forum_threads SET is_attached='1' WHERE id='$th'";
			mysql_query($SQL);
			echo "<script>top.location.href='?act=forum_threads&f={$f}'</script>";
        } elseif(@$_REQUEST['do']=="unattach" && $megaboss>0) {
			$SQL="UPDATE forum_threads SET is_attached='0' WHERE id='$th'";
			mysql_query($SQL);
			echo "<script>top.location.href='?act=forum_threads&f={$f}'</script>";
        } elseif(@$_REQUEST['do']=="delall" && $megaboss>0) {
			$SQL="DELETE FROM forum_threads WHERE id='$th'";
			mysql_query($SQL);
			$SQL="DELETE FROM forum_replies WHERE id_thread='$th'";
			mysql_query($SQL);
			echo "<script>top.location.href='?act=forum_threads&f={$f}'</script>";
        }
#
# add
#
if(strlen($_REQUEST['NewsText'])>4 && $_REQUEST['do'] !== "edit" && $_REQUEST['do'] !== "editthread") {
	...
	if($d['banned']>time()) {
        ...
        $SQL="INSERT INTO forum_replies(id_thread,text,post_date,poster_id)
       ...
       mysql_query($SQL);
		...
		mysql_query($SQL);
        $SQL="UPDATE forum_threads SET last_date='".time()."' WHERE id='$th'";
		mysql_query($SQL);
		echo "<script>top.location.href='?act=forum_threads&f=".$_REQUEST['f']."'</script>";
    }
}
#
# // add
#
if ($_REQUEST['do'] == "edit" || $_REQUEST['do'] == "editthread")
{
	if ($_REQUEST['do'] == 'edit')
		{
			$qe = "SELECT `text` FROM `forum_replies` WHERE `id` = '".$_REQUEST['postid']."' LIMIT 1;";
			$rse = mysql_query($qe);
			$rse2 = mysql_fetch_array($rse);
			$posttext = stripslashes($rse2['text']);
		}

	else if ($_REQUEST['do'] == 'editthread')
		{
			$qe = "SELECT `text` FROM `forum_threads` WHERE `id` = '".$_REQUEST['th']."' LIMIT 1;";
			$rse = mysql_query($qe);
			$rse2 = mysql_fetch_array($rse);
			$posttext = stripslashes($rse2['text']);
		}
...
<td valign=top>
        ...
        <input name="act" type="hidden" value="forum_replies">
        <input name="do" type="hidden" value="<?=$_REQUEST['do']?>">
        <input name="step" type="hidden" value="save_edit">
...
?>
              ...
              <tr><td width="*">
              <textarea ONSELECT="storeCaret(this);" ONCLICK="storeCaret(this);" ONKEYUP="storeCaret(this);" name="NewsText" id="NewsText" rows="15" wrap="VIRTUAL" style="width:95%"><?=$posttext?></textarea>
              </td>
              <td width="180">
...
<a href="JavaScript:AddSmile('budo')"><img border=0 src="/_imgs/smiles/budo.gif"></a>
			  ...
			  <a href="javascript:AddPro('0w', 'NewsText');"><img border="0" src="_imgs/pro/i0w.gif" width="15" height="15"></a><a href="javascript:AddPro('1w', 'NewsText');"><img border="0" src="_imgs/pro/i1w.gif" alt="корсар" width="15" height="15"></a><a href="javascript:AddPro('2w', 'NewsText');"><img border="0" src="_imgs/pro/i2w.gif" alt="сталкер" width="15" height="15"></a><a href="javascript:AddPro('3w', 'NewsText');"><img border="0" src="_imgs/pro/i3w.gif" alt="шахтер" width="15" height="15"></a><a href="javascript:AddPro('4w', 'NewsText');"><img border="0" src="_imgs/pro/i4w.gif" alt="инженер" width="15" height="15"></a><a href="javascript:AddPro('5w', 'NewsText');"><img border="0" src="_imgs/pro/i5w.gif" alt="наемник" width="15" height="15"></a><a href="javascript:AddPro('6w', 'NewsText');"><img border="0" src="_imgs/pro/i6w.gif" alt="торговец" width="15" height="15"></a><a href="javascript:AddPro('7w', 'NewsText');"><img border="0" src="_imgs/pro/i7w.gif" alt="патрульный" width="15" height="15"></a><a href="javascript:AddPro('8w', 'NewsText');"><img border="0" src="_imgs/pro/i8w.gif" alt="штурмовик" width="15" height="15"></a><a href="javascript:AddPro('9w', 'NewsText');"><img border="0" src="_imgs/pro/i9w.gif" alt="специалист" width="15" height="15"></a><a href="javascript:AddPro('10w', 'NewsText');"><img border="0" src="_imgs/pro/i10w.gif" alt="журналист" width="15" height="15"></a><a href="javascript:AddPro('11w', 'NewsText');"><img border="0" src="_imgs/pro/i11w.gif" alt="чиновник" width="15" height="15"></a><a href="javascript:AddPro('12w', 'NewsText');"><img border="0" src="_imgs/pro/i12w.gif" alt="псионик" width="15" height="15"></a><a href="javascript:AddPro('16w', 'NewsText');"><img border="0" src="_imgs/pro/i16w.gif" alt="пси-лидер" width="15" height="15"></a><a href="javascript:AddPro('14w', 'NewsText');"><img border="0" src="_imgs/pro/i14w.gif" alt="пси-кинетик" width="15" height="15"></a><a href="javascript:AddPro('15w', 'NewsText');"><img border="0" src="_imgs/pro/i15w.gif" alt="пси-медиум" width="15" height="15"></a><a href="javascript:AddPro('13w', 'NewsText');"><img border="0" src="_imgs/pro/i13w.gif" alt="каторжник" width="15" height="15"></a><a href="javascript:AddPro('30w', 'NewsText');"><img border="0" src="_imgs/pro/i30w.gif" alt="дилер" width="15" height="15"></a>
                ...
                <input name="add_subm" type="submit" value="Изменить">
	</form>
...
?>
    ...
    <a href='?act=forum_replies&f=<?=$f?>&th=<?=$th?>'><?=stripslashes($thread['title'])?></a>
    </p>
<?
    ...
    $r=mysql_query($SQL);
        ...
        ShowPages($p,$pages,5,'act=forum_replies&f='.$f.'&th='.$th);
        echo '</b></div>';
        ...
        if($p==1) {
            ...
            echo GetClan($thread['clan']).GetUser($thread['user_id'],$thread['user_name'],AuthUserGroup).'<br>
				'.date("d.m.y H:i",$thread['post_date']);
		if ((abs(AccessLevel) & AccessEditForum)) echo "<br><a href='?act=forum_replies&th={$th}&f={$f}&p={$p}&do=editthread'\">изменить</a> ";
            echo '
		...
		if($megaboss>0) echo "<br><a href='#;return false' onclick=\"if(confirm('Удалить пост?')) top.location.href='?act=forum_replies&th={$th}&f={$f}&p={$p}&do=del&postid=".$d['id']."'\">удалить</a> ";
		if ((abs(AccessLevel) & AccessEditForum)) echo "<br><a href='?act=forum_replies&th={$th}&f={$f}&p={$p}&do=edit&postid=".$d['id']."'\">изменить</a> ";
        echo "
			</td><td width=100% valign=top  background='i/bgr-grid-sand.gif'>
        ";
		ParseNews2($d['text'],0);
...
echo '</b></div>';
echo '<div class=vd9>';
if($megaboss>0 && $thread['allow_comments']!=0)
	echo ' <a href="?act=forum_replies&th='.$th.'&f='.$f.'&p='.$p.'&do=close">Закрыть обсуждение</a> ';
elseif($megaboss>0 && $thread['allow_comments']==0)
	echo ' <a href="?act=forum_replies&th='.$th.'&f='.$f.'&p='.$p.'&do=open">Продолжить обсуждение</a> ';

if($megaboss>0 && $thread['is_attached']!=0)
	echo ' / <a href=\'?act=forum_replies&th='.$th.'&f='.$f.'&p='.$p.'&do=unattach\' onclick="return confirm(\'Открепить топик с верхушки форума?\');">открепить</a> ';
elseif($megaboss>0 && $thread['is_attached']==0)
	echo ' / <a href=\'?act=forum_replies&th='.$th.'&f='.$f.'&p='.$p.'&do=attach\' onclick="return confirm(\'Закрепить наверху форума?\');">закрепить</a> ';

if($megaboss>0) echo ' / <a href="?act=forum_replies&th='.$th.'&f='.$f.'&p='.$p.'&do=delall" onclick ="return confirm(\'Удалить весь топик?\');">удалить топик</a> ';


echo '</div>';

if(AuthStatus!=1)
	echo $mess['CantAddReply'].$mess['WantRegister'];
else {
	...
	if($d['banned']>time()) {
		...
		if($thread['allow_comments']==0)
			echo "<h2>Обсуждение закрыто</h2>";
		else {
?>
<SCRIPT src='_modules/xhr_js.js'></SCRIPT>
<SCRIPT src='_modules/news_js.js'></SCRIPT>
