...
<h1>Доступ к системе проверок на чистоту</h1> <?
                ...
                $cur_restr = str_replace("-law_check-", "", $cur_restr);
                ...
                $query = "SELECT `user_name`, `restricted_area` FROM `site_users` WHERE `restricted_area` LIKE '%law_check%' ORDER BY `user_name`";
...
<form name="add_form" method="post" action="?act=law_checkers">
...
//                    ."<td $bg[$y]>$area[$u_restr]</td>"
                    ...
                    ."<td $bg[$y]><a href='?act=law_checkers&del=$u_name' onClick=\"if(!confirm('Вы уверены?')) return false;\">Убрать доступ</a></td></tr>";
