...
// ---------- Вывод архива полиции онлайн-------------	
	...
	//	корректировка
		...
		$text = title("Статистика пребываний МП онлайн<BR>".date("d.m.Y", $time)." 00:00 - ".date("d.m.Y", $time2)." 23:59");
		
		$text .= "<BR><DIV STYLE=\"WIDTH:100%;\">\n";
		
		$text .= "<FORM METHOD=\"GET\" ACTION=\"{_SERVER_NAME_}/{_MAIN_SCRIPT_}\">\n";
		$text .= "<INPUT TYPE=\"hidden\" NAME=\"act\" VALUE=\"mp_stats\">\n";
	...
	*/	$text .= "</DIV>\n";
		...
		$sSQL = "SELECT * FROM `mp_online` WHERE (".$where_date.") ORDER BY `nick` ASC, `login` ASC";
		...
		//	$text .= "<CENTER>Всего: ".$nrows." человек</CENTER>\n";
			...
			// если в базе персов ТЗ не находим ник
				...
				$text .= " <TD VALIGN=top>".$clan." <A HREF=\"{_SERVER_NAME_}/{_MAIN_SCRIPT_}?act=mp_stats&action=view2&time[0]=".$c_time[0]."&time[1]=".$c_time[1]."&time[2]=".$c_time[2]."&time[3]=".$c_time2[0]."&time[4]=".$c_time2[1]."&time[5]=".$c_time2[2]."&login=".$row["nick"]."\">".stripslashes($row3["name"])."</A> [".$row3["level"]."] {_PROF_}".$row3["pro"]."".(($row3["sex"]=="0")?"w":"")."{_/PROF_}<BR>\n";
...
*/		
	...
	*/	$text .= "</DIV>\n";
		...
		$sSQL = "SELECT * FROM `mp_online` WHERE (".$where_date.") AND `nick` = '".anti_sql_injection($login)."' ORDER BY `login` ASC" ;
		$result = mysql_query($sSQL,$connection);
		...
		//	$text .= " <TD><B>Пост</B></TD>\n";
			...
			while($row = mysql_fetch_array($result)){
				...
				if($c_date!=$row["date"]){
					...
					$text .= " <TD ALIGN=\"center\"><A HREF=\"{_SERVER_NAME_}/{_MAIN_SCRIPT_}?act=mp_stats&action=view&time[0]=".date("d", $row["login"])."&time[1]=".date("m", $row["login"])."&time[2]=".date("Y", $row["login"])."&time[3]=".date("d", $row["login"])."&time[4]=".date("m", $row["login"])."&time[5]=".date("Y", $row["login"])."\"><B>".date("d.m.Y", $row["login"])."</B></A></TD>\n";
					$text .= " <TD VALIGN=top>\n";
					
					$sSQL4 = "SELECT * FROM `mp_online` WHERE (`date` = '".$row["date"]."') AND `nick` = '".$row["nick"]."' ORDER BY `login` ASC" ;
					$result4 = mysql_query($sSQL4, $connection);
