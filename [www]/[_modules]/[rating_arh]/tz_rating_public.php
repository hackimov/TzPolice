...
//
	...
	$sSQL = 'SELECT `dept` FROM `sd_cops` WHERE `name` = "'.AuthUserName.'" ORDER BY `chief` DESC LIMIT 1;';
...
//	83 - НачПол, 12 - Отдел прокачек
	if (AuthUserGroup == 100 || $menu_dept_id=="12" || $menu_dept_id=="83"){
		$full_access=1;
	}else
		$full_access=0;
...
///////////////////////////////////////////////////////////////////////////////////
	...
	if($action=='view'){
		$text = rating_screen_view ($full_access, $months_a, $menu, $cid, $time, $connection, $db, $id, $action, $max_level, $pvp);
		...
		$text = rating_screen_view2 ($full_access, $months_a, $menu, $cid, $time, $connection, $db, $id, $action, $max_level, $pvp);

	}elseif($action=='stars'){
		$text = rating_stars_view ($full_access, $months_a, $menu, $cid, $time, $connection, $db, $id, $action, $max_level, $pvp);
