...
//  ?act=cops_stats
//  case 'cops_stats': include "_modules/rating_arh/cops_stats_public.php"; break;	
...
// подключаем все что требуеца
	...
	include_once ($DOCUMENT_ROOT.$path_to_php.'/'.$menu.'/cops_stats_function.php');
...
// начальники
	$sSQL = 'SELECT `chief` FROM `sd_cops` WHERE `user_id` = \''.AuthUserId.'\' LIMIT 1;';
	...
	if ((abs(AccessLevel) & AccessPoliceStats) || $chief=='1' || AuthUserClan=='Tribunal' || AuthUserGroup == 100) {
		$full_access=1;
	}else{
		$full_access=0;
	}
...
///////////////////////////////////////////////////////////////////////////////////
	...
	if($action=='police111'){
		if($full_access==1){
			$text = rating_tz_users_view ($connection, $db);
		}
	
	}elseif($action=='police222'){
		if($full_access==1){
			$text = cops_sostav_view ($connection, $db);
		}
		
	}elseif($action=='view'){
		if($full_access==1){
			$text = police_online_view ($months_a, $time, $connection, $db, $post_names, $cops_action);
		}
		
	}elseif($action=='view2'){
		if($full_access==1){
			$text = police_online_view2 ($login, $months_a, $time, $connection, $db, $post_names, $cops_action);
		}

	}elseif($action=='gistogramma'){
		if($full_access==1){
			$text = police_online_gistogramma ($nicks, $months_a, $time, $time2, $connection, $db);
		}
