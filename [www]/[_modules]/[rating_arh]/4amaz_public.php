...
///////////////////////////////////////////////////////////////////////////////////
	...
	if($action=="add_to_check" || $action=="add_to_check2" || $action=="add_to_check3"){
		...
		===========*/
			...
			if($action=="add_to_check"){
				...
				$text = title("Отправлено в список проверенных - ".$ft." чаров");
			}
			
			if($full_access==2) $text = title("Отправлено на проверку - ".$ft." чаров");
	...
	if($full_access==1 || $full_access==2){
		...
		$menu .= "<A HREF=\"{_SERVER_NAME_}/{_MAIN_SCRIPT_}?act=4amaz&action=on_check\">Отправленные на проверку (".$nrows.")</A>\n";
		$menu .= " | <A HREF=\"{_SERVER_NAME_}/{_MAIN_SCRIPT_}?act=4amaz&action=check\">Проверяемые (".$nrows2.")</A>\n";
		$menu .= " | <A HREF=\"{_SERVER_NAME_}/{_MAIN_SCRIPT_}?act=4amaz&action=checked\">Проверенные (".$nrows3.")</A>\n";
		$menu .= "<HR>\n";
		
		$menu .= "<A HREF=\"{_SERVER_NAME_}/{_MAIN_SCRIPT_}?act=4amaz&action=user_insert\">Добавление на проверку</A>\n";
	//	$menu .= " | <A HREF=\"{_SERVER_NAME_}/{_MAIN_SCRIPT_}?act=4amaz&action=search\">Поиск по базе</A>\n";
