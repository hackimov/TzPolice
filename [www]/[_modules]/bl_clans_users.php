...
<h1>Доступ к модерации клановых ЧС</h1> <?
                ...
                $cur_restr = str_replace("-bl_clans-", "", $cur_restr);
                ...
                $query = "SELECT `user_name`, `restricted_area` FROM `site_users` WHERE `restricted_area` LIKE '%bl_clans%' ORDER BY `user_name`";
...
<form name="add_form" method="post" action="?act=bl_clans_users">
...
//                    ."<td $bg[$y]>$area[$u_restr]</td>"
                    ...
                    ."<td $bg[$y]><a href='?act=bl_clans_users&del=$u_name' onClick=\"if(!confirm('Вы уверены?')) return false;\">Убрать доступ</a></td></tr>";
