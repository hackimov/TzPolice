...
?>
<form action="?act=warez" method="post" name="upload" id="upload" enctype="multipart/form-data">
...
//		            $desc = htmlspecialchars($desc);
				    ...
				    $query = "INSERT INTO `warez` (`id`, `author`, `name`, `version`, `file`, `desc`) VALUES ( '', '".$author."', '".$name."', '".$ver."', '".$new_file_name."', '".$desc."');";
		 		   	if (move_uploaded_file($_FILES['userfile']['tmp_name'], "warez/".$new_file_name))
		            	{
							$result = mysql_query($query) or die("Выполнить запрос не удалось: " . mysql_error());
        		            chmod ("warez/".$new_file_name, 0777);
		                }
        		}
    ...
    elseif (strlen($_REQUEST['link']) > 3)
    	{
		    ...
		    $author = strip_tags($_REQUEST['author']);
			$author = htmlspecialchars($author);
    		$query = "INSERT INTO `warez` (`id`, `author`, `name`, `version`, `file`, `desc`) VALUES ( '', '".$author."', '".$name."', '".$ver."', '".$link."', '".$desc."');";
			$result = mysql_query($query) or die("Выполнить запрос не удалось: " . mysql_error());
        }
}
...
{
    $file_id = $_GET["id"];
	$query = "SELECT * FROM `warez` WHERE `id` = '".$file_id."' LIMIT 1 ;";
    $result = mysql_query($query) or die("Выполнить запрос не удалось: " . mysql_error());
    $row = mysql_fetch_array($result, MYSQL_ASSOC);
    $file_name = $row["file"];
	if (is_file("warez/".$file_name))
    	{
			if (unlink("warez/".$file_name))
				{
		        	$query = "DELETE FROM `warez` WHERE `id` = '".$file_id."' LIMIT 1;";
				    $result = mysql_query($query) or die("Выполнить запрос не удалось: " . mysql_error());
		        }
        }
    else
...
if ($_GET["sec"] == "edit")
{
	$query = "SELECT * FROM `warez` WHERE `id` = '".$_GET['id']."' LIMIT 1 ;";
    $r = mysql_query($query);
...
{
	$query = "UPDATE `warez` SET `file` = '".$_REQUEST['link']."', `author` = '".$_REQUEST['author']."', `name` = '".$_REQUEST['name']."', `version` = '".$_REQUEST['ver']."', `desc` = '".$_REQUEST['desc']."' WHERE `id` = '".$_POST['id']."' LIMIT 1 ;";
    $r = mysql_query($query);
?>
<script>top.location='?act=warez';</script>
<?
...
<?
		$query = "SELECT * FROM `warez` ORDER BY `id` DESC;";
        ...
        while (list($c_id, $c_author, $c_name, $c_ver, $c_file, $c_desc) = mysql_fetch_row($rs))
        	{
            	?>
                  <tr>
                    ...
                    <?
	                   	if (is_file("warez/".$c_file))
                        	{
					$tmpsize = filesize("warez/".$c_file);
                    $size = round($tmpsize/1024);	
                            	?>
				                    <a href="http://www.tzpolice.ru/warez/<?=$c_file?>"><?=$c_name?></a>
				                    <br><b><?=$size?> Kb</b><br>
								<?
                            }
/*                        else
...
*/
                    if(abs(AccessLevel) & AccessArticles)
                    	{
                        ?>
                    <a href="?act=warez&sec=del&id=<?=$c_id?>" onClick="if(!confirm('Вы уверены?')) {return false}">[X]</a> || <a href="?act=warez&sec=edit&id=<?=$c_id?>">[E]</a>
                    	<?
                        }
                        ?>
                    </td>
