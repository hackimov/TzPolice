...
?>
  <a href="?act=total2&sec=rounds">Раунды тотализатора</a><br>
  <a href="?act=total2&sec=bets">Ставки</a><br>
  <a href="?act=total2&sec=results">Результаты</a><br>
  <hr>
<?
...
//обработчик добавления
    ...
    if ($_REQUEST['nomorebets'])
    {
      $query = "UPDATE `total_round` SET `active`='0' WHERE `id`='".$_REQUEST["nomorebets"]."' LIMIT 1;";
      mysql_query($query) or die(mysql_error());
      echo '<i>Прием ставок закрыт</i>';
    }
    if ($_REQUEST['wantbets'])
    {
      $query = "UPDATE `total_round` SET `active`='1' WHERE `id`='".$_REQUEST["wantbets"]."' LIMIT 1;";
      mysql_query($query) or die(mysql_error());
      echo '<i>Прием ставок открыт</i>';
    }
...
//обработчик удаления
    ...
    while ($row = mysql_fetch_assoc($res))
    {
      ...
      echo '<tr>';
        ...
        echo '</td>';
        echo '<td>';
          echo '<a href="?act=total2&sec=rounds&editid='.$row['id'].'">редактировать</a> || <a href="?act=total2&sec=editgames&id='.$row['id'].'">редактировать состав</a> || <a href="?act=total2&sec=rounds&delid='.$row['id'].'" onClick="if(!confirm(\'Вы уверены?\')) {return false}">удалить</a>';
	 if ($row['active'] == 1)
		{
			echo ' || <a href="?act=total2&sec=rounds&nomorebets='.$row['id'].'">закончить прием ставок</a>';
		}
	else
		{
                        echo ' || <a href="?act=total2&sec=rounds&wantbets='.$row['id'].'">открыть прием ставок</a>';
		}
        echo '</td>';
      echo '</tr>';
    }
  ...
  elseif ($_REQUEST['sec'] == 'bets')
  {
    ...
    while ($row = mysql_fetch_assoc($res))
    {
      echo '<tr>';
        ...
        echo '</td>';
        echo '<td>';
          echo '<a href="?act=total2&sec=makebet&id='.$row['id'].'">сделать ставку</a> || <a href="?act=total2&sec=betlist&id='.$row['id'].'">список ставок</a>';
//          echo '<a href="?act=total2&sec=rounds&editid='.$row['id'].'">редактировать</a> || <a href="?act=total2&sec=editgames&id='.$row['id'].'">редактировать состав</a> || <a href="?act=total2&sec=rounds&delid='.$row['id'].'" onClick="if(!confirm(\'Вы уверены?\')) {return false}">удалить</a>';
        echo '</td>';
      echo '</tr>';
    }
...
<?
      ...
      while ($row = mysql_fetch_assoc($r))
      {
        ...
        echo '<tr>';
          echo '<td width=50 background="i/bgr-grid-sand1.gif"><a href="?act=total2&sec=betlist&id='.$_REQUEST['id'].'&delid='.$row['id'].'" onClick="if(!confirm(\'Вы уверены?\')) {return false}">[X]</a></td>';
