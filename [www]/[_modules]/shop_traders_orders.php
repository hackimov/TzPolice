<h1>Заказы интернет-магазина "военторгЪ Traders edition"</h1>
...
if (AuthStatus==1 && (substr_count(AuthUserRestrAccess, "traders_shop") > 0 || substr_count(AuthUserRestrAccess, "traders_shop") > 0)) {
...
if ($d=mysql_fetch_array($r)) $permission=$d['factory'];
	...
	if(@$_REQUEST['cancelID']>0) {
		...
		$SQL="SELECT status FROM traders_orders WHERE id='".addslashes($_REQUEST['cancelID'])."'";

		$d=mysql_fetch_array(mysql_query($SQL));

		if ($d['status']==1) {

			$SQL="DELETE FROM traders_orders WHERE id='".addslashes($_REQUEST['cancelID'])."'";
	...
	if(@$_REQUEST['getID']>0) {
		...
		$SQL="UPDATE traders_orders SET status='0' WHERE id='".addslashes($_REQUEST['getID'])."'";
...
if($permission) {
	...
	if(@$_REQUEST['acceptID']>0) {
		...
		$SQL="SELECT customer, resources, factory FROM traders_orders WHERE id='".addslashes($_REQUEST['acceptID'])."'";
...
/*
		...
		if ($error) {
			...
			echo "<script>top.location='?act=traders_orders&error=".$error."';</script>";
...
*/
			...
			$SQL="UPDATE traders_orders SET status='2', accept='".AuthUserName."' WHERE id='".addslashes($_REQUEST['acceptID'])."' AND status<'4' AND status>'0'";
...
//		}
	...
	if(@$_REQUEST['executeID']>0) {
		...
		$SQL="UPDATE traders_orders SET status='3', t2='".time()."', execute='".AuthUserName."', stored_in='".addslashes($_REQUEST['place'])."' WHERE id='".addslashes($_REQUEST['executeID'])."'";

		mysql_query($SQL);

		$query = "SELECT `customer` FROM `traders_orders` WHERE `id` = '".addslashes($_REQUEST['executeID'])."' LIMIT 1;";
        ...
        while (list ($tmpf, $tmpi, $tmpc) = mysql_fetch_row($res))

        	{
		        ...
		        $query = "INSERT INTO `traders_history` (`item`, `date`, `customer`, `quantity`, `plant`, producer) VALUES ('".$tmpi."', '".time()."', '".$client."', '".$tmpc."', '".$tmpf."', '".AuthUserName."');";
	...
	if(@$_REQUEST['packedID']>0) {
		...
		$SQL="UPDATE traders_orders SET status='4', t2='".time()."', give_out='".AuthUserName."', stored_out='".addslashes($_REQUEST['place'])."' WHERE id='".addslashes($_REQUEST['packedID'])."'";
	...
	if(@$_REQUEST['delID']>0) {
		...
		$SQL="DELETE FROM traders_orders WHERE id='".addslashes($_REQUEST['delID'])."'";
...
<?$d=mysql_fetch_array(mysql_query("SELECT COUNT(*) AS maxx FROM traders_orders WHERE customer='".AuthUserId."' AND status=0"));?>
...
<form name="ready_form" method="post" action="?act=shop_traders_orders">
...
<form name="packed_form" method="post" action="?act=shop_traders_orders">
...
$SQL="SELECT i.*, u.id AS UID, u.user_name AS UName, u.clan AS UClan FROM traders_orders i LEFT JOIN site_users u on u.id=i.customer WHERE i.status='1' ORDER by i.t1 DESC";
...
<td <?=$bg?> nowrap valign=top><b><?=$d['id']?> [<a href='#; return false;' onclick="if(confirm('Вы уверены?')) top.location='?act=shop_traders_orders&delID=<?=$d['id']?>'">X</a>]</b></b></td>
...
<a href="?act=shop_traders_orders&acceptID=<?=$d['id']?>">оплата получена</a><br>

<a href="?act=shop_traders_orders&delID=<?=$d['id']?>">удалить заказ</a>
...
$SQL="SELECT i.*, u.id AS UID, u.user_name AS UName, u.clan AS UClan FROM traders_orders i LEFT JOIN site_users u on u.id=i.customer WHERE i.status='2' ORDER by i.t1 DESC";
...
$SQL="SELECT i.*, u.id AS UID, u.user_name AS UName, u.clan AS UClan FROM traders_orders i LEFT JOIN site_users u on u.id=i.customer WHERE i.status='3' ORDER by u.id";
...
$SQL="SELECT i.*, u.id AS UID, u.user_name AS UName, u.clan AS UClan FROM traders_orders i LEFT JOIN site_users u on u.id=i.customer WHERE i.status='4' ORDER by u.id";
...
<input name="act" type="hidden" value="shop_traders_orders">
...
$r=mysql_query("SELECT DISTINCT o.customer, u.user_name FROM traders_orders o LEFT JOIN site_users u ON u.id=o.customer ORDER BY u.user_name");
...
$SQL="SELECT i.*, u.id AS UID, u.user_name AS UName, u.clan AS UClan FROM traders_orders i LEFT JOIN site_users u on u.id=i.customer WHERE (i.status='0' OR i.status='3') AND i.customer='".htmlspecialchars($_REQUEST['view_history'])."' ORDER by i.t1 DESC";
...
$SQL="SELECT * FROM traders_orders WHERE customer='".AuthUserId."' ORDER BY status DESC, t1 DESC";
...
if($d['status']=='1') {
	...
	echo "заказ отправлен<br>Вы еще можете его <a href='?act=shop_traders_orders&cancelID={$d['id']}'>отменить</a>.";
...
elseif ($d['status']=='4') {
	...
	if($_REQUEST['getID']==$d['id']) {
		...
		echo "<script>top.location='?act=traders_shop';</script>";

	}

	echo "заказ готов<br><a href='#; return false;' onclick=\"if(confirm('Вы получили заказ?')) top.location='?act=shop_traders_orders&getID={$d['id']}'\">сделайте пометку</a> о получении.";
...
<? if($permission) { echo("."); } else {?>
<center>
<br><br>
<b>Заказы других пользователей:</b><br>
</center>
<table width=97% cellpadding=5 align=center>
<?
$SQL="SELECT i.*, u.id AS UID, u.user_name AS UName, u.clan AS UClan FROM traders_orders i LEFT JOIN site_users u on u.id=i.customer WHERE i.status='1' ORDER by i.t1 DESC";
$r=mysql_query($SQL);
if(mysql_num_rows($r)>0) {
...
?>
<tr>
<td <?=$bg?> nowrap valign=top><b><?=$d['id']?></b></b></td>
<td <?=$bg?> nowrap valign=top><?=GetClan($d['UClan']).GetUser($d['UID'],$d['UName'],AuthUserGroup)?><br><?=date("d.m.y H:i",$d['t1'])?></td>
<td <?=$bg?> align=center valign=top><b><?=$factory['name']?></b></td>
<td <?=$bg?> valign=top><?=$order?></td>
</tr>
<?}?>
</table><br>
