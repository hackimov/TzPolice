...
<h1>Пользователи Top Secret - Military Police</h1> <?
		...
		$query = "SELECT `user_name`, `restricted_area` FROM `site_users` WHERE `restricted_area` LIKE '%military%' ORDER BY `user_name`";
...
<form name="add_form" method="post" action="?act=user_military">
...
//                    ."<td $bg[$y]>$area[$u_restr]</td>"
                    ."<td $bg[$y]><a href='?act=user_military&del=$u_name' onClick=\"if(!confirm('Вы уверены?')) return false;\">Убрать доступ</a></td></tr>";
                $y++;
                if ($y>1) {$y = 0;}
            }
        ?>
