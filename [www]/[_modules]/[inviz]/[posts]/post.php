<h1>Отчетность по посту на ЦП</h1>
<?php
...
if($_REQUEST['a']=="TakePost") {
	...
	if ($rec=mysql_fetch_array($rez)) {
		...
		$SQL2="SELECT * from posts_report where post_g=0 and city=1";
		$r2=mysql_query($SQL2);
		$ex= mysql_num_rows($r2);
		if($ex==0){
			$SQL="INSERT INTO posts_report (id_user,post_t,id_week,city) values ('".AuthUserId."','".time()."','".date("W")."','1')";
			mysql_query($SQL);
			$SQL="DELETE FROM posts_order WHERE id_user='".AuthUserId."' AND post='1'";
			mysql_query($SQL);
		}
		echo "<script>top.location.href='?act=post';</script>";
	}
}

$SQL="SELECT p.*, u.id AS UID, u.user_name AS Uname FROM posts_report p LEFT JOIN site_users u ON u.id=p.id_user where city=1 ORDER BY id DESC LIMIT 1";
$r=mysql_query($SQL);
...
<form method="GET">
<input name="act" type="hidden" value="post">
...
<?php
  ...
  $norm_r=mysql_query(
    'SELECT post_g, post_t FROM posts_report WHERE (id_week='.date("W").') AND (city=1) AND (id_user=\''.
...
while($d=mysql_fetch_array($r)) {
	...
	?></td>
	<td nowrap align="center"><a href="?act=post&a=dell_record&eid=<?php echo $d_id; ?>">удалить</a></td>
	</tr>
	<?php
}
?>
...
<?php
$t=time()+600;
$SQL="select * from posts_order_entry where post=1 and id_user='".AuthUserId."' and post_date_from<'".$t."' and post_date_from>'".time()."' and confirmed=0";
$r=mysql_query($SQL);
while($d=mysql_fetch_array($r)) {
extract($d,EXTR_PREFIX_ALL,"d");
...
if($_REQUEST['a']=="make_confirm") {
	$SQL="UPDATE posts_order_entry set confirmed=1 WHERE id_user='".AuthUserId."' AND post='1' and id='".$cid."'";
	mysql_query($SQL);
	echo "<script>top.location.href='?act=post';</script>";
}

if($_REQUEST['a']=="dell_record") {
	$SQL="DELETE FROM posts_order_entry WHERE id_user='".AuthUserId."' AND post='1' and id='".$eid."'";
	mysql_query($SQL);
	echo "<script>top.location.href='?act=post';</script>";
}
...
if($_REQUEST['a']=="make_record") {
	...
	$SQL="INSERT INTO posts_order_entry (id_user,post,post_date_from,post_date_to,confirmed) values ('".AuthUserId."','1','".$stime."','0','0')";
	mysql_query($SQL);
	echo "<script>top.location.href='?act=post';</script>";
}
...
if($_REQUEST['a']=="in_order") {
	...
	$SQL="INSERT INTO posts_order (id_user,post,post_time,must_take_post) values ('".AuthUserId."','1','".time()."',0)";
	mysql_query($SQL);
...
if($_REQUEST['a']=="GetPost") {
   $SQL="UPDATE posts_report set post_g='".time()."', comment=' ".htmlspecialchars($_REQUEST['reason'])."' WHERE city=1 and id='$last_id' and post_g=0";
   mysql_query($SQL);
   echo "<script>top.location.href='?act=post';</script>";
}
if($_REQUEST['a']=="GivePost") {
   $SQL="UPDATE posts_report set post_g='".time()."' WHERE city=1 and id_user='".AuthUserId."' and post_g=0";
   mysql_query($SQL);
	...
	$SQL="SELECT
		...
		p.post_time
        FROM posts_order p
        LEFT JOIN site_users u ON u.id=p.id_user WHERE p.post=1 order BY p.post_time ASC limit 1";
  $r=mysql_query($SQL);
  //echo mysql_num_rows($r);
  while($d=mysql_fetch_array($r)) {
        extract($d,EXTR_PREFIX_ALL,"d");
		$mt=time()+600;
		$SQL="UPDATE posts_order set must_take_post='".$mt."' WHERE post=1 and id_user='".$d_id_user."' and post_time='".$d_post_time."'";
   		mysql_query($SQL);
...
echo "
      <a href='?act=post&stats=week'>сводная статистика за текущую неделю</a>
      (сейчас идет <b>".date("W")."</b> неделя)  <BR>
      <a href='?act=post&stats=week2'>сводная статистика за текущую неделю + оазис</a>
      (сейчас идет <b>".date("W")."</b> неделя)  <BR>
      <a href='?act=post&stats=day'>сводная статистика за день</a>
...
<?
  ...
  $SQL="SELECT (p.post_g-p.post_t) AS time, p.post_g, p.post_t, p.id_user, u.user_name FROM posts_report p LEFT JOIN site_users u ON u.id=p.id_user WHERE city=1 and (p.post_t>$daystamp AND post_t<$daystamp2) AND p.post_g>0 ORDER BY p.post_t";
  $r=mysql_query($SQL);
  echo "
  ...
  $SQL="SELECT SUM(p.post_g-p.post_t) AS ttime FROM posts_report p WHERE city=1 and (p.post_t>$daystamp AND post_t<$daystamp2) AND p.post_g>0";
...
<?
  ...
  <table cellpadding=3 width=50% cellspacing=3>";
  $SQL="SELECT sum(p.post_g-p.post_t) AS time, p.id_user, u.user_name FROM posts_report p LEFT JOIN site_users u ON u.id=p.id_user WHERE city=1 and $_sql AND p.post_g>0 GROUP BY p.id_user order by u.user_name";
  $r=mysql_query($SQL);
  echo "
  ...
  $SQL="SELECT SUM(p.post_g-p.post_t) AS ttime FROM posts_report p WHERE city=1 and p.id_week='$W' AND p.post_g>0 AND p.post_t > '$old'";
  $d=mysql_fetch_array(mysql_query($SQL));
  $TotalTime=round($d['ttime']/60);
  ...
  if($show_accidents==1) {
          ...
          $SQL="SELECT p.comment, p.id_user, u.user_name, p.post_g, p.post_t FROM posts_report p LEFT JOIN site_users u ON u.id=p.id_user WHERE city=1 and p.id_week='$W' AND p.post_t > '$old' AND p.post_g>0 ORDER BY p.post_g ASC";
...
<?
  ...
  $SQL="SELECT
        ...
        FROM posts_report p
        LEFT JOIN site_users u ON u.id=p.id_user
        WHERE city=1 and $_sql AND p.post_g>0 GROUP BY p.id_user
...
<?
  ...
  $UsrStr=$dt['user_name'];
// query
  $SQL="SELECT p.post_t, (p.post_g-p.post_t) AS cont FROM posts_report p WHERE city=1 and p.id_week='$W' AND p.post_t > '$old' AND p.post_g>0 AND p.id_user='$userid'";
  $r=mysql_query($SQL);
  echo "
...
<table cellpadding=3 width=50% cellspacing=3>
 <?
  ...
  $SQL="SELECT p.post_t,p.post_g, (p.post_g-p.post_t) AS cont FROM posts_report p WHERE city=1 and p.id_week='$uW' AND p.post_t > '$old' AND  p.id_user='".AuthUserId."'";
	//p.post_g>0 AND
  $r=mysql_query($SQL);
  echo "
  ...
  $SQL="SELECT SUM(p.post_g-p.post_t) AS ttime FROM posts_report p WHERE city=1 and p.id_week='$uW' AND p.post_t > '$old' AND p.post_g>0 AND p.id_user='".AuthUserId."'";
  $d=mysql_fetch_array(mysql_query($SQL));
