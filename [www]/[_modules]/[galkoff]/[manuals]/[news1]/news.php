				...
				if (empty($news_search_author) && empty($news_search_words)) {
					...
					if (!empty($news_search_author)) {
						...
						for ($i = 0; $i < $count_split; $i++) {
							...
							$news_search_where .= ' u.user_name LIKE \'%'.$news_search_author_split[$i].'%\'';
							if ($i < ($count_split-1)) {
		...
		while($d=mysql_fetch_assoc($r)) {
			...
			if(AuthUserGroup>1 || (abs(AccessLevel) & AccessNewsEditor)) {
				...
				echo '
		            ...
		            <a onclick="if(confirm(\'Удалить новость из базы?\')){top.location=\'?act=news&IdNews='.$d['id'].'&do=delete\'}" href="javasrcipt:{}">удалить</a> /
		       ';
			}
