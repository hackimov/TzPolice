<h1>Заявки на восстановление контроля над персонажем (администрация)</h1>
...
function tz_DoFSCommand(command, args) {
	...
	if (command == "OK")
		...
		req2.caching = false;
		req2.open('POST', '_modules/backends/pers_adm_list.php', true);
...
function list()
	{
		...
		req99.caching = false;
		req99.open('POST', '_modules/backends/pers_adm_list.php', true);
...
function accept()
	{
		...
		req.caching = false;
		req.open('POST', '_modules/backends/pers_adm_do.php', true);
...
function showPers()
	{
		...
		req.caching = false;
		req.open('POST', '_modules/backends/pers_adm.php', true);
