...
if(mysql_num_rows($r)<1) echo $mess['NewsNotFound'];
    ...
    echo "
		...
		<td height='20' background='i/bgr-grid-sand.gif'><p><img src='i/bullet-red-01a.gif' width='18' height='11' hspace='5'><strong>Обсуждение статьи: </strong> <span class='top-header'>{$d['title']}</span></p></td>
		</tr><tr><td>
		</tr></table>
	";
    ...
    if(mysql_num_rows($r)<1) echo $mess['NoComments'];
        ...
        for($i=0;$i<mysql_num_rows($r);$i++) {
            ...
            ";
            if(abs(AccessLevel) & AccessArticles)  echo "<a href='#;return false' onclick=\"if(confirm('Удалить комментарий?')){top.location='?act=data_comments&DataId={$d[id_data]}&IdComment={$d[id]}&do=delete&p={$p}'}\">удалить</a> / ";
...
ShowPages($p,$pages,4,"act=data_comments&DataId={$_REQUEST['DataId']}");
echo "</b></div>";
if((abs(AccessLevel) & AccessArticles) && $AllowComments!=0) echo "<div>[ <a href='?act=data_comments&do=close&DataId=".$_REQUEST['DataId']."&p=$p'>Закрыть обсуждение</a> ]</div>";
if((abs(AccessLevel) & AccessArticles) && $AllowComments==0) echo "<div>[ <a href='?act=data_comments&do=open&DataId=".$_REQUEST['DataId']."&p=$p'>Продолжить обсуждение</a> ]</div>";
