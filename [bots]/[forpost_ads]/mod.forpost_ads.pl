...
sub pl_fab_on_idle
{
	...
	my $wait5 = time-($bot->{pl_fab}{lastscan5}||0);
...
#				msg($alertnum);
				...
				msg("$bot->{login}: SEND MESSAGE TO CHAT $alertnum");
...
sub pl_fab_on_data
{
	...
	if ($key->{name} eq "STATUS")
		...
		if (defined $key->{data}{clan}) {
			my $fname = "$logs/forpost-".strftime("%Y%m%d%H%M%S", localtime);
