...
$home = "/home/sites/police/bots/prison";
$logs = "$home/logs";
...
unless ($home eq ".") {
	...
	if (open my $file, "$home/prison_bot.pid") {
		my $pid = <$file>;
		chomp $pid;
		close $file;
		exit if -d "/proc/$pid";
...
sub chat_in
{
	...
	my @spec_phrases = (
		...
		'Если я один выйду на дорогу, то ей богу звездонёт!',
		'Иной раз бояться-то нечего, кроме последствий своего бездействия, но страх парализует, и ничего с этим не поделать.',
		'Как хотелось бы многим покинуть самих себя в минуту опасности!',
		'Оттуда порой не возвращаются и в переносном смысле. Я знаю множество таких людей',
		'Бомбометание закончено. Жертв и разрушений нет.',
		...
		'А на Прохоровском поле 1200 танков схватились в рукопашную.',
		'Не только защита, но и нападение бывает глухим.',
		'Ни для какого другого дела мужчины не объединяются так быстро, как для убийства других мужчин.',
		'Ничто так не накаляет обстановку, как холодная война.',
		'Опять горячие головы втягивают нас в холодную войну.',
		'Подавая сигналы в рог, Будь всегда справедлив, но строг.',
		'Старики объявляют войну, а умирать идут молодые. :old:',
		'Тело снайпера боится. :naem:',
		...
		'Достигнет ли человек когда- нибудь такого уровня морали, что создаст для кочевников передвижные тюрьмы?'
	);
		...
		if ($key->{name} eq "KEY") {
			...
			foreach (split ",", $list) {
				if ($_) {
					...
					if (lc($login) ne "верещагин" && lc($login) ne "dimon" && lc($login) ne "cheaterz" && ($pro<13 || $pro>13) && ($pro<7 && $pro>9) && ($pro<30 || $pro>30))
						{	
							chat_msg($bot, "private [radio] Алярм!!! Нарушитель на каторге: [$login] ($level уровень)");
							
							chat_msg($bot, "private [radio] ".$spec_phrases[int(rand(25))]."");

							my $fname = "/home/sites/police/www/_modules/prison/".strftime("%Y%m%d", localtime).".txt";
							if (open my ($file), ">>$fname")
								{
									print $file strftime("%H:%M:%S", localtime)." - [pers nick=".$login." level=".$level." pro=".$pro."]";
									print $file "<br>\n";
									close $file;
									chmod (0666, "$fname");
								}
			...
			#push @loc_cache, qq~<A loc="$bot->{chat}{loc_name}" login="$login" clan="$clan" level="$level" pro="$pro" woman="$woman"/>~;
			if (lc($login) ne "верещагин" && lc($login) ne "dimon" && lc($login) ne "cheaterz" && lc($clan) ne "police" && ($pro<13 || $pro>13) && ($pro<7 || $pro>9) && ($pro<30 || $pro>30))
				{
					chat_msg($bot, "private [radio] Алярм!!! Нарушитель на каторге: [$login] ($level уровень)");
