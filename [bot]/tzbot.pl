...
$debug = 0;
open(FFF, ">$home/tzbot.log"); close(FFF);
...
require "$home/mod.pl_online.pl";
require "$home/mod.locator.pl";
require "$home/mod.wars.pl";
...
msg("TZBOT started");
# AVE 30/06/08 = load up names of allowed cops (may be redoing it in txt way?)
open my ($fcops), $home."/anames.txt";
chomp(@{$conf{history_allowed}} = <$fcops>);
close $fcops;
tie @otherhistoryqueue, 'Tie::File', $home."/queue.txt";
#AVE end
...
sub imsg {
	my $msg = shift;
	if(open my ($file), ">>$logs/qbot.html") {
...
sub client_in
{
	...
	foreach my $key (@{$xml}) {
		...
		if ($key->{name} eq "KEY") {
			...
			#if ($bot->{login} eq "HONY") {
			#	my $fname = "$home/input";
			#	open my ($file), ">>$fname.log";
			#	print $file "$data";
			#	close $file;
			#}
...
my @stan_messages;
push(@stan_messages,"Судью на мыло! Начпола на беляши, рагу и мясной салат :crazy: ");
push(@stan_messages,"Товарисч, вы почему не работаете? Ааа начальник...вот оно как :crazy:");
push(@stan_messages,"Плати зарплату чаще :mad: и больше :tongue:");
push(@stan_messages,"Спорим не блокнешь? :tongue:");
push(@stan_messages,"Даёшь приём через постель :protest:");
push(@stan_messages,"Шухер пацаны, тут менд! :str:");


my @exposure_messages;
push(@exposure_messages,"Боюсь, боюсь :str:");
push(@exposure_messages,"Что прикажете, моя госпожа? :mol:");
push(@exposure_messages,"Ваш верный пёс готов служить :yessir:");
push(@exposure_messages,":hug2: ой :shy:");
push(@exposure_messages,":shuffle: Вы сегодня так прекрасны! Как всегда :)");
push(@exposure_messages,":flowers: Конфискованный букет, вынес за пазухой :red: ");
push(@exposure_messages,":cop: Рыжий рыжий конопатый, дал молчаночку лопате :joy: ");
push(@exposure_messages,":mad: Я устал. Я ухожу. ");
push(@exposure_messages,"Девушка, дайте рутовый пароль от вашего сердца :shy: ");
push(@exposure_messages,"Продам, прокачаю персонажа :joy: Недорого :hehehe:");
push(@exposure_messages,"Блок, шок, чс, маринад... Доброе утро ТЗшник! :crazy:");
push(@exposure_messages,"XXX: *напеваед про себя* первым делом, первым делом - сотый леевел! Ну а девушки - потом! | YYY: да ты, как я погляжу, оптимист xD");
push(@exposure_messages,"Классическое надругательство над замерзшими и стремившимися домой людьми: `Котик, пока не разделся, сходи в магазин`");
push(@exposure_messages,"Нет здесь никого, не на кого нападать :tongue:");
push(@exposure_messages,"Мне кажется, что местная каптча пытается мне что-то сказать, о чем-то предупредить, но я как на зло не знаю, на каком языке она со мной разговаривает.");
push(@exposure_messages,"Я мужик, я хочу всё контролировать! Но жена не разрешает :cry2:");
push(@exposure_messages,"Возьмите меня на руки и покачайте :cry2: Я же нежное и доброе создание :sad:");
push(@exposure_messages,"У нас с китежем война? Помогите, я напал! :crazy:");
push(@exposure_messages,"Извините, я занят. По всем вопросам обращайтесь к себе :crazy:");
push(@exposure_messages,":mad: ааа нельзя же так пугать! Я нечаянно эликса блокнул :wall:");
push(@exposure_messages,":lick: Щас молчанку кину.");
push(@exposure_messages,"Насяльникееее хешельмеее! Я мана мала мольчянк кидаль, блокь делаль, каторга випускаль, а ти ругаисся даааа :(");
push(@exposure_messages,"Коп. Рыжий коп. :gent:");
...
sub chat_private
{
	...
	if ($text =~ /^\s*private \[(.+?)\]\s*(.*)$/) {
		if (lc $1 eq lc $bot->{login} && $2 ne "") {
			...
			if('Exposure passion' eq $from) {				if($cmd =~ m/:kulich:/i || $cmd =~ m/:boogi:/i) {					my $rnd2 = int rand(10);
					if($rnd2 > 3) {						my $rnd1 = int rand values @exposure_messages;						my $mess1 = @exposure_messages[$rnd1];	                	chat_msg($bot,"private [clan] private [$from] $mess1");
                	} else {                		my $rnd1 = int rand values @stan_messages;
						my $mess1 = @stan_messages[$rnd1];
	                	chat_msg($bot,"private [clan] private [stan79] $mess1");
	                	chat_msg($bot,"private [clan] :mad: Эта ни я сказал. Не блочь меня.");
                	}
				}
			}
			...
			if (($cmd eq "getinf")||($cmd eq "getbtl"))
			{
                if ($allow_his){
					...
					if ($text_msg=~ /get(inf|btl)\s+(.+?)\s+(\d\d\.\d\d\.\d\d)\s+(\d\d\.\d\d\.\d\d)$/){
					    ...
					    if ($@){
	            	    	chat_msg($bot,"private [$from] Что-то у вас не так в написании дат... Я это расшифровать не могу... :dkn:");
	            	    	$correct=0;
					    }else{
							$enddate=~/(\d+)\.(\d+)\.(\d+)/;
					    ...
					    if ($@){
        	    	    	...
        	    	    	chat_msg($bot,"private [$from] Дата начала сканирования больше даты конца сканирования! :mad:");
            		    	$correct=0;
						    }
						    if ($startsecs>$currtime){
	            	    		chat_msg($bot,"private [$from] Не все могут смотреть в завтра! (Дата начала сканирования больше текущей даты!) :mad:");
    	        	    	$correct=0;
						    }
						    if ($endsecs>$currtime){
            		    	chat_msg($bot,"private [$from] Не все могут смотреть в завтра! (Дата конца сканирования больше текущей даты!) :mad:");
            	    		$correct=0;
					    	}
					    }
					    if ($correct){

							my ($sday, $smon, $syear) = split /\./, $startdate;
							my $ustartdate = mktime( 0, 0, 0, $sday, $smon-1, $syear+100);
							my ($eday, $emon, $eyear) = split /\./, $enddate;
							my $uenddate = mktime( 0, 0, 0, $eday, $emon-1, $eyear+100);

							# отсечка в 120 дней.
							if (($uenddate-$ustartdate)> 10368000) {
								$ustartdate = $uenddate - 10368000;
								$startdate = strftime("%d.%m.%y", localtime $ustartdate);
								chat_msg($bot,"private [$from] Глубина сканирования превышает 120 дней. Дата начала установлена в ".$startdate);
							}

							push(@otherhistoryqueue,$getcmd."|".$from."|".$victim."|".$startdate."|".$enddate."|".time);
    	        	    	chat_msg($bot,"private [$from] Ваш запрос поставлен в очередь. Ваш номер в очереди - ".scalar @otherhistoryqueue.". Ждите ответа.");
            	    	}
                	}else{
