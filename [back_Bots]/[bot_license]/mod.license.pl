use strict;
our $home2 = "/home/sites/police/bot_license";
...
sub load_alerts
	{
		open (BASE, "<$home2/cops.txt");
		my @base = <BASE>;
		close (BASE);
		unlink("$home2/cops.txt");
		touch("$home2/cops.txt");
		my $q = 0666;
		my $w = "$home2/cops.txt";
		chmod($q, $w);
		for (my $i=$#base; $i>=0; $i--)
...
sub pl_fab_on_idle
{
	...
	my $wait2 = time-($bot->{pl_fab}{lastscanalert}||0);
				...
				foreach my $person (keys %alerts)
					{
						if ($person eq "-1")
							...
							{
								chat_msg($bot, "private [$person] $alerts{$person}");
							}
						#client_out($bot, qq~<PDA p2="$alerts{$person}" p1="$person" id="19782776292.1" />~);
