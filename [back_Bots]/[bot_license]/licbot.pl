...
$home = "/home/sites/police/bot_license";
$logs = "$home/logs";
...
unless ($home eq ".") {
	...
	if (open my $file, "$home/licbot.pid") {
		my $pid = <$file>;
		chomp $pid;
		close $file;
		exit if -d "/proc/$pid";
...
sub load_alerts
	{
		open (BASE, "<$home/cops.txt");
		my @base = <BASE>;
		close (BASE);
		unlink("$home/cops.txt");
		touch("$home/cops.txt");
		my $q = 0666;
		my $w = "$home/cops.txt";
		chmod($q, $w);
...
sub chat_private
{
	...
	if ($text =~ /^\s*private \[(.+?)\]\s*(.*)$/) {
		if (lc $1 eq lc $bot->{login} && $2 ne "") {
			...
			if (defined $clients{$from} && ($cmd eq "status" || $cmd eq "статус"))
				{
					chat_msg($bot, "private [$from] $clients{$from}");
					$iscmd = 1;
				}
			else
				{
					if ($cmd eq "status" || $cmd eq "статус")
						{
							chat_msg($bot, "private [$from] Информация по персонажу '$from' отсутствует. :sad:");
							$iscmd = 1;
						}
                    elsif ($cmd eq "checkme" || $cmd eq "проверь")
                    	{
							#chat_msg($bot, "private [$from] Заявка на обычную проверку персонажа '$from' отправлена на обработку. Примерно через 10 минут Вы сможете отслеживать состояние заявки, сказав мне в приват 'статус' (без кавычек). С Новым Годом и Рождеством :swans:");
							chat_msg($bot, "private [$from] Халява кончилась. Всем спасибо, все свободны :farewell:");
                            #my $fname = "$logs/wantcheck";
				#			if (open my ($file), ">>$fname.txt")
				#				{
				#					print $file $from;
				#					print $file "\n";
				#					close $file;
                                #   chmod (0666, "$fname.txt");
                                    $iscmd = 1;
								#}
                        }
					else
