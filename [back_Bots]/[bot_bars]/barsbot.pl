...
$home = "/home/sites/police/bot_bars";
$logs = "$home/logs";
...
unless ($home eq ".") {
	...
	if (open my $file, "$home/barsbot.pid") {
		my $pid = <$file>;
		chomp $pid;
		close $file;
		exit if -d "/proc/$pid";
...
#				$sock = IO::Socket::INET->new(PeerAddr => "city1.timezero.ru", PeerPort => 5190, Blocking => 1);
				$sock = IO::Socket::INET->new(PeerAddr => "213.219.216.160", PeerPort => 5190, Blocking => 1);
			} else {
#				$sock = IO::Socket::INET->new(PeerAddr => "city1.timezero.ru", PeerPort => 5190);
				$sock = IO::Socket::INET->new(PeerAddr => "213.219.216.160", PeerPort => 5190);
			}
		...
		my $flag = $sock->recv($data, POSIX::BUFSIZ);
	        ...
	        unless(defined $flag && length $data) {
				...
				delete $bot->{client};
	        		...
	        		msg("$bot->{login}  Chat: Disconnected");
